...
function query(a) {
	...
	$(document).ready(function (){
		$.post("<%=path%>/bd_findAll",{buyId:buyId,currentPage:pagecur,pageSize:pagesize},
			function(data){//Servlet执行完之后执行方法，data表示的servlet返回数据内容
	 				var object = eval(data);//将字符串转换成json类型
	 				var showT = "<table class='table table-hover text-center'>  <tr> <th>采购明细编号</th> <th>采购单编号</th> <th>采购商品名</th> <th>商品数量</th> <th>商品价格</th><th>商品总价</th><th>操作</th> </tr>";
	 				for(var i = 1;i<object.length;i++){
	 					var item = object[i];
	 					showT = showT+"<tr><td>"+item.bdId+"</td><td>"+item.buyId+"</td><td>"
	 					+item.proname+"</td><td>"+item.buynum+"</td><td>"+item.price+"</td><td>"
	 					+item.total+"</td><td><a class='button border-main' href='#' onclick='goUpd(" 
	 					+ item.bdId + ")'>更新</a><a class='button border-red' href='#' onclick='goDel("
					+ item.bdId + ")'>删除</a></td></tr>";
	 				}
	 				showT = showT + "</table>";
